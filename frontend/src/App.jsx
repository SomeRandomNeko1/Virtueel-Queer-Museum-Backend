...
export default function App({ token, onLogout, addLog }) {
  ...
  const filtered = items
    ...
    .filter(x => {
      ...
      return [x.Naam, x.Beschrijving, x.Type, x.Auteur].some(v => v && v.toLowerCase().includes(s))
    })

  const currentPage = navItems.find(n => n.id === page)

  const handleItemSaved = useCallback((updated) => {
              ...
              {showMenu && (
                ...
                >
                  ...
                  <div className="p-1">
                    ...
                    >
                      <FiLogOut size={13} /><span>Uitloggen</span>
                    </button>
                  </div>
                </div>
              )}
...
function HomeView({ items, loading, err, types, typeFilter, setTypeFilter, picked, setPicked, isMobile, token, addLog, onItemSaved, onItemDeleted}) {
  ...
  return (
    <div className="flex gap-5.5 flex-1 min-h-0 flex-col md:flex-row h-auto md:h-[calc(100vh-116px)]">
      <div className={`flex-1 min-w-0 overflow-y-auto pr-1 ${editPanelOpen ? "hidden md:block" : ""}`}>
        {types.length > 0 && (
        ...
        {err && <p className="text-ink text-[13px] text-center mt-10">Fout: {err}</p>}
        ...
        <div className="w-full md:w-170 shrink-0 flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-3 shrink-0">
            <div className="flex items-center gap-2 text-muted">
              <FiEdit2 size={13} />
              <span className="font-body text-[12px] font-semibold tracking-[.06em] uppercase">
  ...
  return (
    ...
    >
      <div className="h-30 bg-warm-bg flex items-center justify-center overflow-hidden relative">
        {imageSrc
          ? <img src={imageSrc} alt={item.Naam}
        ...
        {active && (
          <div className="absolute inset-0 bg-black/5 flex items-end justify-end p-2">
      ...
      <div className="px-3.25 pt-2.75 pb-3.25">
        <p className="font-display text-[15px] font-semibold text-ink mb-0.5 leading-tight">
          {item.Naam || "Naamloos"}
        </p>
        {item.Auteur && (
          <p className="font-body text-[11px] text-muted mb-1">{item.Auteur}</p>
        )}
        {item.Type && (
